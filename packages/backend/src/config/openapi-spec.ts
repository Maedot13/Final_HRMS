  ...
  paths: {
    ...
    '/api/v1/employees/{id}': {
      ...
      patch: {
        ...
        description: 'Update employee profile. \\n\\n**Simple Rule for RBAC:**\\n- **Campus Admins/HR**: Can only update **operational fields** (department, position, location, contact info). Financial fields (salary, pay grade, tax info) in the request will be automatically stripped.\\n- **University Admins (Central HR)**: Can update **all fields**, including financial and system data.',
        parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'integer' } }],
        requestBody: {
          content: {
            'application/json': {
              schema: {
                ...
                properties: {
                  ...
                  employmentType: { type: 'string', enum: ['PERMANENT', 'CONTRACT', 'PROBATION'] },
                  contractStartDate: { type: 'string', format: 'date-time' },
                  contractEndDate: { type: 'string', format: 'date-time' },
                  grossSalary: { type: 'number', format: 'float', description: 'Restricted to University Admins' },
                  payGrade: { type: 'string', description: 'Restricted to University Admins' },
                  tinNumber: { type: 'string', description: 'Restricted to University Admins' },
                  pensionNumber: { type: 'string', description: 'Restricted to University Admins' },
    ...
    '/api/v1/users/{id}/role': {
      patch: {
        ...
        description: 'Update user role. Campus Admins manage local roles. University Admins have cross-campus oversight.',
        parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'integer' } }],
    ...
    '/api/v1/users/{id}/status': {
      patch: {
        ...
        description: 'Toggle account access. Campus Admins manage local status. University Admins have cross-campus oversight.',
        parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'integer' } }],
